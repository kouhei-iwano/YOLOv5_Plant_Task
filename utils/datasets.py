...
class LoadImagesPlantTest:  # for testing plant single-class
    def __init__(self, path, root='', img_size=640, stride=32):
        ...
        assert len(files[0].split(','))==2, 'Possible wrong test format. The correct test row should be <img_path>, <class_ID>'
        ...
        images = [x for x in files if x.split('.')[-1].lower() in IMG_FORMATS]
        videos = [x for x in files if x.split('.')[-1].lower() in IMG_FORMATS]
        ni, nv = len(images), len(videos)
    ...
    def __next__(self):
        if self.count == self.nf:
            raise StopIteration
        path = self.files[self.count]
        label = int(self.labels[self.count])
