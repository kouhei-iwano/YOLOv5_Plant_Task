...
Usage:
    $ python path/to/train.py --data coco_tomato.yaml --weights yolov5s.pt --img 640
"""

""" 
Train Plant Dataset:
    $ python train.py --img 1472 --batch 32 --epochs 30 --data cucumber_online_9class.yaml --weights yolov5l6.pt --name cucumber_online_l6

Finetuning:
    $ python train.py --img 1472 --batch 64 --epochs 100 --data cucumber_online_9class.yaml --weights runs/train/cucumber_online_l6/weights/epoch_80.pt --name cucumber_online_l6_ft

If break training run:
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in [-1, 0]:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                # Save every n epoch
                if(epoch % opt.every_n == 0):
                    epoch_save = 'epoch_%d.pt' % epoch
                    epoch_save = w / epoch_save
                    torch.save(ckpt, epoch_save)
                    strip_optimizer(epoch_save)
...
def parse_opt(known=False):
    ...
    parser.add_argument('--every_n', type=int, default=1, help='Save model at every n epochs')
